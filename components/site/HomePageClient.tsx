...
import { CATEGORIES, getPublishedGuides, PRODUCTS, HERO_IMAGE, type CategoryKey, type Article } from "@/lib/catalog";

type Props = {
  initialGuides?: Article[];
};

export function HomePageClient({ initialGuides }: Props) {
  ...
  const guidesList = initialGuides && initialGuides.length > 0 ? initialGuides : getPublishedGuides();
  const featuredGuide = guidesList[0];
